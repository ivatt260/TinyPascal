...
Sept 20 2025
	- code cleanup.
	- starting to use command line for input/output and options (i.e.
	  no piping anymore).
	- doing some expression optimizations. This is a precursor for 
	  more efficient code as we do records and multi-dimensional arrays.
	- code (currently commented out) inspectJumps - to see where code flow is.

Sept 5 2025
	- copy in the 1982 expression opt code from print-outs.

July 30 2025

	- fix 1802 assembler to support following 
		  call function change( no lv passed in)


	- version changed to V07



April 2025: 
	- version V06 "system" released
...
{TinyPascal compiler with code generation}
    ...
    fct = (ver, 
          lit, lod, sto, opr, 
          bbnd, stk, int, pcal, fcal, pret, fret, jmp, jpc, tot, tin, xit);
    ...
    table: array [0..txmax] of
           record 
              ...
              case kind: obj of
                ...
                simpType_def: (size,lowerBound,upperBound:Longint;
...
procedure parseCommandLine;
  ...
  *)
  if paramCount() <> 1 then begin
    writeln();
    writeln('TinyPascal compiler - currently expect only 1 filename on command line;'); 
    writeln('e.g.  "TinyPascal testprog.mod"');
    writeln();
    Halt(1);
  end;
  ...
  {writeln ('working on reassigning input');}
  {use IOResult to check for errors}
  {$i-}
  {if 1 parameter, try opening it as input}
  ...
  {writeln('COMMAND LINE, paramStr1:',paramStr(1));}

  Assign (Input,paramStr(1));
  {writeln('IOResult after assign:',IOResult);}

  if IOResult <> 0 then begin
    writeln ('can not open input file at step 1:',paramStr(1));
  end else begin
    {writeln('step 2 for reassigning input');}
    reset(Input);
    {writeln ('past reset input');}
    {writeln('EInOutError after reset:',EInOutError);}
    {writeln('IOResult after reset:',IOResult);}
    if IOResult > 0 then begin
      {writeln('assignment error after reset');}
      writeln ('can not open input file for some strange reason:',paramStr(1));
      Halt(1);
    end;
...
(********************************************************)
{ Did we smash through the bounds of uint16s? }
procedure boundsCheckUint16(tos, progptr: Longint);
  begin
    {is TOS out of range for a uint16?}
      if (tos<0) or (tos>65535) then begin
        writeln ('uint16 issue, at cx:',progPtr:0,
          ' value out of range, is:',tos:0);
        Halt(1); 
      end;
    end;
           
(********************************************************)
{ Perform Uint16 math operations in optimization phase and/or
  in interpret phase}

function oprDoUint16(ax, progPtr, tos,tos1:integer) : integer;
  var rv, tmp: integer;

  begin {oprDoUint16}
    rv := -1; {set it to something...}
    case ax of
           opr_plus_uint16: rv := tos + tos1;
           opr_minus_uint16: rv := tos - tos1;
           opr_mul_uint16:   rv := tos * tos1;
           opr_div_uint16:   rv := tos div tos1;
           opr_mod_uint16:   rv := tos mod tos1;
           opr_eql_set16,   
           opr_eql_uint16:   rv := ord(tos = tos1);
           opr_neq_set16,
           opr_neq_uint16:   rv := ord(tos <> tos1);
           opr_lss_uint16:   rv := ord(tos < tos1);
           opr_geq_uint16:   rv := ord(tos >= tos1);
           opr_gtr_uint16:   rv := ord(tos > tos1);
           opr_leq_uint16:   rv := ord(tos <= tos1);
           opr_and_uint16:   if (tos<>0) and (tos1<>0) then rv :=1 else rv:=0;
           opr_or_uint16:    if (tos<>0) or (tos1<>0) then rv :=1 else rv:=0;

           opr_incl_set16: begin {  = 29; "<=" operator}
              tmp := tos;
              {AND the inclusion with the full;}
              tos := wordWideOp(opr_and_set16,tos1, tos);
    ...
    else writeln ('not coded yet - oprDoUint16 ax:',ax:0);
    end; {case}
    boundsCheckUint16(rv,progPtr);
    oprDoUint16 := rv;
  end;
...
(********************************************************)
procedure outTo_A18__Assembler;
  type instr = set of fct;
  var i: integer;
  var tfOut: Text;
  var instr2, instr3, instr4: instr;
  ...
  begin {outTo_A18__Assembler}
    {list code generated for this block}

    {mnemonic, level, ax}
    instr4 := [lod, fcal,pcal, sto,tot,stk]; 

    {mnemonic, ax}
    instr3 := [lit,ver,bbnd,int,jmp,jpc];
    ...
    rewrite(tfOut);
    writeln('  Writing ASM02 formatted assembler file to "assemblerOut.asm"');
...
procedure getsym;
   ...
   begin if cc = ll then
      begin if eof(input) then
                 begin write(' program incomplete'); Halt(1)
                 end;
         ll := 0; cc := 0; 
         write(cx: 5,' ');
...
(****************************************)
{see if we do jump-to-jump code for possible optimizations}
{
procedure inspectJumps;
  var mycx, jumpto: integer;
  begin
    for mycx := 0 to cx-1 do 
      with code[mycx] do begin
        writeln('inspectJumps, cx:',mycx:0,' opr:',omnemonic[fn]);
        if (fn = jmp) or (fn = jpc) then 
          writeln ('jmp at:',mycx:0,'type:',omnemonic[fn],' dest is:',ax:0,' type:',omnemonic[code[ax].fn]);
      end; 
  end;
}
...
(********************************************************)
procedure gen(x: fct; y,z: integer; gentype: integer);
begin if cx > cxmax then
           begin write(' program too long'); Halt(1);
           end;
   with code[cx] do
...
procedure exprGen( x: fct; y,z: integer);
  begin
    {writeln ('exprGen:  ',mnemonic[x],' y:',y:2,' z:',z:2);}
    {write('exprGen: ');
    case x of
      ver: begin
             writeln ('VER - huh?');
      ...
      lod: begin
             writeln ('LOD, lev:',y:2,' offset:',z:2);
           end;
      sto: begin
             writeln ('STO - huh?');
           end;
...
(************************************************************)
    function expression(fsys: symset; 
         ...
         var optCX : integer; {for expression optimization}
    ...
    with table[i] do
      begin {2}
        ...
        if sym = lparen then begin {3}
          ...
          exprGen(stk, 0, DlSlRa_func + nparams+1);
        end  {3} 
        else if nparams>0 then error (lparenExpected);


      exprGen(fcal, lev-level, adr);
    end; {2}
end; {1}
...
(************************************************************)
                ...
                with table[i] do
                 case kind of
                    const_def: begin
                      ...
                      {write('exprGenLit in factor_ident: '); 
                      ...
                      if expressionSet then exprGen(lit,0,setToBit16(value))
                      else exprGen(lit, 0, value);
                      end;
                    var_def: begin
            ...
            procedure factor_charString;
              ...
              begin
                ...
                {trying to ger rid of a compiler warning; this does nothing}
                intFromChar := intFromChar + 2;
                {writeln ('factor_charString, have char of value ',intFromChar);}

                exprGen(lit, 0, byte(constCharArray[constStringStart]));
                getsym
            ...
            procedure factor_lbracket;
              ...
              begin {factor_lbracket}
                ...
                if isSetType(expectedTyp) then begin {1}
                  ...
                  exprGen(lit,0,0); {start the value with NULL, or in elements}
                    ...
                    begin {2}
                      ...
                      repeat
                        ...
                        if sym = dotdot then begin {3}
                          ...
                          exprGen (opr,0,opr_dotdot_set16);
            ...
            procedure factor_peek_etc;
              ...
              begin {factor_peek_etc} {1}
                ...
                if sym = lparen then begin {2}
                  ...
                  case mysym of
                    peeksym: exprGen(opr,0,opr_peek);
                    ordsym:  {do nothing};
                    chrsym:  {do nothing};
                    predsym: begin
                               exprGen(lit,0,1); {pred: change by 1}
                               exprGen(opr,0,opr_minus_uint16); {subtract}
                             end;
                    succsym: begin
                               exprGen(lit,0,1); {succ: change by 1}
                               exprGen(opr,0,opr_plus_uint16); {add}
                             end;
                  end; {case}
...
(************************************************************)
         begin {term} 
           ...
           while sym in termbegsys do begin {while}
             ...
             if expressionSet then begin
               if termop=times then exprGen(opr,0,opr_and_set16) {equiv to opr_intersection_set}
               else writeln ('term, expressionSet, should not be here');
             end else begin
               if termop=times then exprGen(opr,0,opr_mul_uint16) 
               else if termop = slash then exprGen(opr,0,opr_div_uint16)
               else if termop = divsym then exprGen(opr,0,opr_div_uint16)
               else if termop = modsym then exprGen(opr,0,opr_mod_uint16)
               else if termop = andsym then exprGen(opr,0,opr_and_uint16)
             end;
...
(************************************************************)
      begin {simpleExpression}
       ...
       { write('BEGIN simpleExpression, sym:'); printSym(sym);}
        ...
        if addop = minus then exprGen(opr, 0,opr_neg_uint16);
              ...
              if addop in [plus,minus] then begin {+-}
                ...
                if expressionSet then begin {is a set}
                  ...
                  if addop = plus then 
                    ...
                    exprGen(opr,0,opr_invert_set16);
                    exprGen(opr,0,opr_and_set16);
                  end
                  ...
                  if addop=plus then
                     exprGen(opr,0,opr_plus_uint16)
                  else exprGen (opr,0,opr_minus_uint16)
                end {if addop in +-}
              end 
    ...
    { is this an operation that we can do?}
    function canDoThisOp(op:integer): boolean;
      begin
        case op of
           {Two stack entry input, one stack output}
           opr_plus_uint16, 
           opr_minus_uint16,
           opr_mul_uint16,
           opr_div_uint16,
           ...
           opr_incl_set16:
             canDoThisOp := true;
           else canDoThisOp := false;
         end; {case}
      end;

    {take a copy of cx at the beginning of an expression for optimization}
    procedure exprMark();
      begin
        optCX := cx;
        {writeln ('exprMark, cx = ',cx);}
      end;

    {go through this expression, and see if we can concatenate constants}
    procedure optimizeExpression();
      var p1,p2, op: integer;
      var shuffleCounter: integer;
      var inspectstart: integer;
      var foundOne : boolean;

      begin
        {writeln;writeln ('optimizeExpression, cx = ',cx,' optCX = ',optCX);
        listcode;}

        if optCX <= (cx+3) then
          begin {canopt}
            inspectstart := optCX;
            {writeln ('going to do opt while, optCX',optCX:3,' cx ',cx:3);}

            while inspectStart <= (cx-2) do
              begin {while1}
                repeat
                  foundOne := false;

                  {writeln ('in inspectStart while loop, inspectStart:',inspectStart:2);}
                  if (code[inspectStart].fn = lit) and
                   (code[inspectStart+1].fn = lit) and
                   (code[inspectStart+2].fn = opr) then
                   begin {lit lit opr}
                     {writeln('found one! before shuffle inspectStart:',inspectStart);}
                     {listcode;
                     writeln ('optimize this');}
                     p1 := code[inspectStart].ax;
                     p2 := code[inspectStart+1].ax;
                     op := code[inspectStart+2].ax;
                
                     if canDoThisOp(op) then
                       begin {can do this op}
                         {keep track of numbers of pcode removed}
                         optimizeRemovedPCODE := optimizeRemovedPCODE+2;
                         {constant math}
                         code[inspectStart].ax := oprDoUint16( op, inspectStart, p1, p2);

                         {shuffle code along here}
                         for shuffleCounter := inspectStart+1 to cx do 
                           code[shuffleCounter] := code[shuffleCounter+2];

                         {reduce the cx count, and start at "optCX" again}
                         cx := cx - 2;
                         inspectstart := optCX;
                         foundOne := true;
                       end; {can do this op}
             ...
             inspectStart := inspectStart+1;
           end; {while1}
         end; {canopt}
                            
            
         {writeln; listcode; writeln('fin optimizeExpression'); writeln;}

      end; {optimizeExpression}
         ...
         {writeln('after if have sym in exprbegsys, LHS:',expRTypLHS:3,' RHS:',expRTypRHS:3);}
          relop := sym; 
           ...
           if (expRTypRHS>0) and (expRTypRHS<txmax) then
             if table[expRTypRHS].kind = set_def then begin {2}
               ...
               if table[expRTypLHS].kind <>
                 ...
                 exprGen(opr, 0,opr_flip_tos16);
                 exprGen(opr, 0,opr_int_toSet16);
               end; {3}
             end; {2}
           ...
           if expressionSet then begin {2}
             {set expression}
             case relop of
               insym: exprGen (opr, 0, opr_and_set16);
               eql:   exprGen (opr, 0, opr_eql_set16);
               neq:   exprGen (opr, 0, opr_neq_set16);
               leq:   exprGen (opr, 0, opr_incl_set16);
               geq:   begin
                        {like leq, just flip params}
                        exprGen (opr, 0, opr_flip_tos16);
                        exprGen (opr, 0, opr_incl_set16);
                      end;
               else begin {3}
             ...
             {16 bit expression}
             case relop of
               eql: exprGen(opr, 0, opr_eql_uint16);
               neq: exprGen(opr, 0, opr_neq_uint16);
               lss: exprGen(opr, 0, opr_lss_uint16);
               geq: exprGen(opr, 0, opr_geq_uint16);
               gtr: exprGen(opr, 0, opr_gtr_uint16);
               leq: exprGen(opr, 0, opr_leq_uint16);
               else begin {3}
          ...
          with table[i] do
            begin
              ...
              if sym = lparen then begin
                getsym;
                {create space for parameters}
                gen(stk, 1, DlSlRa_proc,unknownType);
                repeat
                ...
                until (pcount >= nparams);
                {remove space for parameters}
                gen(stk, 0, DlSlRa_proc+nparams,unknownType);
      
                if sym=rparen then getsym else error(rparenExpected);
      
              end;
              gen(pcal, lev-level, adr,unknownType);
            end
      end;
               ...
               begin {3}
                 ...
                 with table[i+1] do
                   begin {4}
                     ...
                     gen(sto, lev-level, adr,unknownType)
                   end; {4}
               end {3}
        ...
        begin 
         ...
         if sym = elsesym then 
           begin
             getsym;
             cx2 := cx;
             gen(jmp, 0, 0,noType); {cx2 points to his jmp for fixup}
              ...
              if sym = becomes then
                begin
                  ...
                  if table[i].kind = var_def then begin
                    {store initial value}
                    with table[i] do gen(sto, lev-level, adr,unknownType);
                  ...
                  with table[i] do gen(lod, lev-level, adr,unknownType);
                  ...
                  if todowntosym = tosym then
                    gen(opr,0,opr_leq_uint16,unknownType) {tops>=tops+1}
                  else
                    gen(opr,0,opr_geq_uint16,unknownType); {tops<=tops+1}

                  { this is the jpc to exit, have to touchup}
                  cx2 := cx; 
                  gen(jpc,0,2000,noType);
                  if sym = dosym then
                    begin
                      getsym;
                      statement(fsys);
                  ...
                  with table[i] do gen(lod, lev-level, adr,unknownType);
                  gen(lit,0,1,unknownType); {for increment/decrement count}
                  if todowntosym = tosym then
                    gen(opr,0,opr_plus_uint16,unknownType) {add}
                  else
                    gen(opr,0,opr_minus_uint16,unknownType); {subtract}
                  with table[i] do gen(sto, lev-level, adr,unknownType);

                  {back up to top}
                  gen(jmp,0,cx1,noType);
                end {to downto}
        ...
        if sym=lparen then begin
          ...
          repeat
            ...
            if sym=comma then getsym;
              if sym = ident then begin
                ...
                gen(tin,IO_char,0,unknownType);
                with table[readDest] do gen(sto, lev-level, adr,unknownType);
                getsym;
              end;
          until sym <> comma;
          ...
          if sym=rparen then begin
            ...
            if readReadln=IO_Readln then begin
              ...
              gen(tot,IO_charString,0,unknownType);
            end;
	...
	if sym = writelnsym then writeWriteln:=IO_Writeln;
        ...
        if sym=lparen then begin
          ...
          if sym = charString then begin
            ...
            getsym;
            gen(tot,IO_charString,constStringStart,unknownType);
          end else begin
            ...
            if returnedTyp =  uint16Type then
              gen(tot,IO_uint16,0,unknownType)
            else if returnedTyp = charType then
              gen(tot,IO_char,0,unknownType)
            else begin
...
(************************************************************)
      ...
      begin 
         ...
         returnedTyp := expression([dosym]+fsys,typ,normalParsing);
         cx2 := cx; 
         gen(jpc, 0, 0,noType); {cx2 points to this jpc for fixup}
           ...
           begin
             ...
             if sym = comma then
               begin
                 ...
                 gen(opr,0,opr_poke,unknownType);
                 if sym = rparen then
                   getsym
               end
             else error (commaExpected);
...
begin {block} 
   ...
   if lev > 0 then
     begin
       if parentType = func_def then
         gen(fret, 1, 0,unknownType) {return}
       else
         gen(pret, 0, 0,unknownType) {return}
     end
   else 
     begin
       gen (xit, 0, 0,unknownType); {exit program}
...
procedure interpret;
   ...
   var progPtr,
      ...
      s: array [1..stacksize] of Longint; {datastore}
   ...
   begin 
      ...
      while reqlev > 0 do
         begin 
           {writeln ('reqlev:',reqlev:0,' b1:',b1:0,' s[b1]:',s[b1]:0);}
           b1 := s[b1]; reqlev := reqlev - 1;
...
{
  print the interpreter stack - for debugging
  procedure printStack();
    var tmp: integer;
    begin
    for tmp := 1 to tops+3 do begin
      writeln('s[', tmp:0, '] ', s[tmp]:0);
    end;
  end;
}
...
begin 
   writeln('  Start TinyPascal interpreter');
   count := 1;
   ...
   repeat
      ...
      with i do  begin
        write('ic:',ic:0);
        write('  progPtr: ',progPtr:0);
        write('  instr:',omnemonic[fn]);
        write(' tops:',tops:3,' dl:',dynamicLink:0);
        writeln();
        printStack;
      ...
      fret: begin
                 ...
                 tops := tops+1;
                 s[tops] := rv;
                 //writeln('ret f, tos:',tops:2, 'pp:',progPtr:0,' dl:',dynamicLink:0);
               end;
      ...
      pret: begin
                 ...
                 progPtr := s[tops + 3]; 
                 dynamicLink := s[tops + 2];
                 {writeln('ret p, tos:',tops:2, 'pp:',progPtr:0,' dl:',dynamicLink:0);}
           end;
      ...
      opr: case ax of {operator}
           ...
           :  
              begin 
                 tops := tops - 1; 
                 {eg: s[tops] := s[tops] plus  s[tops + 1];}
                 s[tops] := oprDoUint16(ax,progPtr,s[tops],s[tops+1]);
              end;
      ...
      lod: begin tops := tops + 1; 
             {writeln('lod: lv:',lv:0,' base:',base(lv):0, ' ax:',ax:0,
               ' val:',s[base(lv)+ax]);
             writeln('....., exact address is:',base(lv)+ax);
             printStack();
             }
             s[tops] := s[base(lv) + ax]
      ...
      pcal,fcal: begin {generate new block mark}
              {writeln('cal; start, dl:',dynamicLink:0);}
              ...
              dynamicLink := tops + 1; 
              progPtr := ax;
              {writeln ('cal; fin st+1:',s[tops+1]:0, ' st+2:', s[tops+2]:0,' st+3:',s[tops+3]:0,
               ' dl:',dynamicLink:0,' progPtr:',progPtr:0); }
           end;
      int: begin
             tops := tops + ax;
           end;
      ...
      ver: begin
           {versions are 16 bit numbers, representing 2 ascii characters}
           write('  PCode Version: ',chr(ord(ax div 256)),chr(ord(ax)));
           writeln;
           end;
...
begin {main program}
  ...
  {Version "07" June 5  2025}
  {decimal 48 is ascii '0', 55 is '7'}
  gen(ver,0,48*256+55,unknownType);
  ...
  if errcount=0 then begin
    {for inspection of pcode jumping:  inspectJumps;}
    outTo_A18__Assembler;
    if optimizeRemovedPCODE > 0 then
      writeln('  Optimization:  pcode changed/removed:',optimizeRemovedPCODE:0);
    interpret;
